...
export function Caroserie({ images }: { images: { src: string; alt?: string }[] }) {
  return (
    <div className="w-full overflow-x-auto lg:overflow-visible">
      <div
        className="grid grid-cols-3 lg:grid-cols-2 gap-4 p-4 min-w-max lg:min-w-0 touch-pan-x select-none"
      >
        {images.map((img, i) => (
          <motion.div
            key={i}
            ...
            whileTap={{ scale: 0.98 }}
            className="relative w-40 h-28 lg:w-full lg:h-64 rounded-2xl overflow-hidden shadow-xl flex-shrink-0 lg:flex-shrink"
          >
            <Image
              src={img.src}

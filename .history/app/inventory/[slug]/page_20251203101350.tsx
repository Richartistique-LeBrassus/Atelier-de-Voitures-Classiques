...
import MetadataInjector from "@/components/ui/MetadataInjector";
...
function SpecRow({ k, v }: { k: string; v: string }) {
  return (
    <div className="flex justify-between py-2 text-sm border-b border-white/6 space-y-3">
      <div className="font-medium text-neutral-950">{k}</div>
      <div className="font-medium font-mono">{v}</div>
    </div>
  );
}
...
export function generateMetadata({ params }: { params: { id: string } }) {
  return {
    title: `Inventory | ${params.id}`,
    description: `Explore detailed specifications, history, and gallery for vehicle ID: ${params.id}.`,
    openGraph: {
      title: `Inventory | ${params.id}`,
      description: `Full details for vehicle ${params.id}`,
    },
  };
}
...
export default function ProductDetails({ vehicle }: { vehicle: Vehicle }) {
  ...
  return (
    <div className="min-h-screen bg-white text-neutral-950 uppercase py-24 md:py-44">
      ...
      <MetadataInjector
        title={`Inventory – ${demo.id}`}
        description={demo.description?.slice(0, 150)}
      />
          ...
          <motion.div 
            ...
            >
              <div className="sticky top-6 uppercase text-neutral-950 sm:shadow sm:p-4 
                mt-6 lg:mt-0 p-1 md:py-12"
              >
                <div className="flex items-center justify-between pb-5">
                  <div className="h-fit space-y-4">
                    <h1 className="text-5xl 
                    font-bold tracking-tighter font-mono pb-4">
      ...
      <motion.section  
        ...
        transition={{ duration: 1.2, ease: "easeOut" }}
        viewport={{ once: true, amount: 0.3 }}
        className="max-w-[1400px] mx-auto sm:px-6 lg:px-10 py-12 lg:py-20"
      >
        <Coroserie gallery={SAMPLE_VEHICLE.gallery} />
      </motion.section>
      ...
      mx-auto sm:px-6 lg:px-10 py-12 lg:py-20 uppercase">
        ...
        >
          ...
          p-6 border border-white/6 sm:shadow-xs">
            <h2 className="text-3xl font-extrabold">STORY & HISTORY</h2>
            <p className="mt-4 leading-relaxed line-clamp-5 text-sm font-mono font-semibold mb-16">
              {demo.description}
            </p>
          ...
          <aside className="bg-linear-to-b py-5 px-6 sm:shadow-xs">
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex gap-3">
                <input
                  ...
                  className="w-full bg-white/5 p-2 placeholder:text-slate-400 font-mono font-medium"
                />
                <button className="px-4 py-2 font-extrabold uppercase font-mono italic">Calculate</button>
              </div>
            </div>
          </aside>
        </motion.section>
      ...
      <footer className="border-t border-white/6 mt-12">
        <motion.div
         ...
         flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <div className="text-lg mb-3">Ready to take the next step?</div>
            <div className="text-2xl font-extrabold font-mono">Contact our sales team — viewings available daily</div>
          </div>

          <div className="flex gap-3 uppercase text-xl mt-3">
            <button className="px-6 py-3 font-semibold uppercase 
            font-mono flex items-center gap-2">
              <Phone className="w-5 h-5" />
              Call
            </button>
          
            <button className="px-6 py-3 uppercase font-mono flex 
            items-center gap-2">
...
const SAMPLE_VEHICLE: Vehicle = {
  ...
  //gallery: [
  //  { src: "/imgs/h2.jpg", alt: "Macan front" },
  ...
  gallery: [
    { src: "/imgs/h2.jpg", alt: "Macan front" },
    { src: "/imgs/kunstsammlung.jpg", alt: "Macan side" },
    { src: "/imgs/targa.jpg", alt: "Macan interior" },
    { src: "/imgs/madel.jpg", alt: "Macan rear" },
    { src: "/imgs/bg.jpg", alt: "Macan interior" },
    { src: "/imgs/herwhite.jpg", alt: "Macan rear" }      
  ],
  features: [ "British Elegance","Canadian Based 1960 DB4", "Zagato Certified", "Show Winning Quality"],
  specs: {
    Power: "395 BHP",
    Engine: "4.7 litre twin plug",
    "0-100 km/h": "6.2 s",
    TopSpeed: "245 km/h",
    Weight: "1,220 kg",
  },
...
export function Coroserie({
  gallery,
}: {
  gallery: { src: string; alt?: string }[];
}) {
  return (
    <div className="w-full overflow-x-auto px-auto mx-auto">
      <div
        className="
          inline-flex 
          snap-x snap-mandatory 
          touch-pan-x 
          w-max lg:w-full lg:gap-1
        "
      >
        {gallery.map((img, i) => (

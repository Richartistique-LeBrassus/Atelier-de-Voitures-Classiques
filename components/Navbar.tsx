  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.aside
            className="fixed top-0 left-0 h-full w-3/4 sm:w-1/3 bg-neutral-50 text-neutral-900 z-50 flex flex-col justify-center px-12 space-y-12"
            initial={{ x: "-100%" }}
            animate={{ x: 0 }}
            exit={{ x: "-100%" }}
          ...
          >
            <nav className="space-y-6">
              {["Home", "The Atelier", "Heritage", "Restorations", "Contact"].map(
                (item) => (
                  <motion.a
                    key={item}
                    href="#"
